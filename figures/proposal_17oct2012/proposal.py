...
class Radials(object):
    ...
    def __init__(self,sim,simname,time,corner,boxlen,wind,frac=1.0):
        self._simname = simname
        self._time = time
        self._sim = sim
        self._snap = sim.FindAtTime(time)
        self._outnum = self._snap.iout
        self._folder = "plots/"+folder+"/"
        self._boxlen = boxlen
        self._corner = corner
        self._wind = wind
        self._frac = frac
    ...
    def Run(self):
        ...
        # Plot slice
        # TODO: RE-ENABLE; DISABLED AS SLICES DIDN'T EXIST FOR THE DATA NEEDED
        self.PlotSlices(self._sim.Location(), self._outnum,\
                            "density",self._boxlen,self._frac)
        # Output log
    ...
    # Make plot of energy in the same figure
    def PlotSlices(self, folder, outnum,hydroname,boxlen,frac=1.0):
        savename = self._folder+"slice_"+self._simname+"_"+hydroname
        plotslice.plot(outnum,folder,savename,self._boxlen,self._corner,frac)
        plt.clf()
...
def run(runtype="sn",imgtype="pdf"):
    ...
    if windrt:
        ...
        runtitles = ["windrtsn"]
        snonly = [False]
    simnames = ["09_Thornton_corner_hllc_c0p5"]
    simtitles = ["thornton"]
    boxlens = list()
    sims = list()
    names = list()
    ...
    for sim, name, corner, boxlen in zip(sims, names, corners,boxlens):
        for time, tstr in zip(times, timestrs):
            ...
            plotter = Radials(Simulation(name,sim),name+"_"+tstr,time,\
                                  corner,boxlen,wind,frac)
            plotter.Run()
